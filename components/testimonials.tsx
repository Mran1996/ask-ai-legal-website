...
const CLIENT_RATINGS: Record<ClientId, number> = {
  sandra: 5,
  marcus: 5,
  priya: 4.5,
  keisha: 5,
...
function StarRating({ rating, light }: { rating: number; light?: boolean }) {
  const empty = light ? "text-[#C5A059]/30" : "text-[#C5A059]/35"
  const filled = "fill-[#C5A059] text-[#C5A059]"
    ...
    <div className="flex gap-0.5" aria-label={`${rating} out of 5 stars`}>
      {Array.from({ length: 5 }).map((_, i) => {
        ...
        if (isHalf) {
          return (
            <span key={i} className="relative inline-block h-3.5 w-3.5 shrink-0">
              <Star className={`absolute h-3.5 w-3.5 ${empty}`} aria-hidden />
              <span className="absolute inset-y-0 left-0 w-1/2 overflow-hidden" aria-hidden>
                <Star className={`h-3.5 w-3.5 ${filled}`} />
              </span>
            </span>
          )
        }
        ...
        return (
          <Star
            key={i}
            className={`h-3.5 w-3.5 shrink-0 ${isFilled ? filled : empty}`}
            aria-hidden
          />
        )
      })}
...
function ReviewCardContent({
  ...
  return (
    <>
      <StarRating rating={client.rating} light />

      <blockquote className="mt-3 text-sm leading-relaxed text-gray-700 sm:text-[0.9375rem]">
        &ldquo;{client.quote}&rdquo;
      </blockquote>

      <footer
        className="mt-5 flex items-start gap-3 border-t border-navy/10 pt-4"
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 flex items-center gap-1 text-[10px] font-medium uppercase tracking-wide text-gold-dark">
...
export function Testimonials() {
  ...
  const clientOutcomes = useMemo<ClientOutcome[]>(
    () =>
      CLIENT_IDS.map((id) => {
  ...
  const count = clientOutcomes.length
      ...
      <div className="container-main">
        ...
        <div className="relative mt-12 flex flex-col items-center sm:mt-14">
          ...
          <div className="mt-6 flex justify-center gap-2">
            {clientOutcomes.map((client, index) => (
              <button
                key={client.id}
                type="button"
                onClick={() => goTo(index)}
